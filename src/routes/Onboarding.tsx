...
export default function Onboarding({ initial, platform, onComplete }: { initial: AppSettings; platform: string; onComplete: () => Promise<void> }) {
  ...
  return <main className="onboarding">
    ...
    <section className="onboarding-stage">
      ...
      {step === 1 ? <div className="onboarding-copy"><div className="step-icon"><KeyRound /></div><p className="eyebrow">Step 1</p><h1>Connect OpenRouter</h1><p>Dictum uses your key directly. At roughly $0.003 per minute, an hour of dictation costs about 18 cents. The key never enters app storage or logs.</p><Card className="connect-card"><Field label="OpenRouter API key" hint="Stored securely in Keychain, Credential Manager, or Secret Service."><Input type="password" autoFocus placeholder="sk-or-v1-…" value={apiKey} onChange={(e) => { setApiKey(e.target.value); setKeyOk(false); setKeyError(""); }} />{keyError ? <span className="field-error">{keyError}</span> : null}</Field><Button busy={checking} disabled={!apiKey || keyOk} onClick={() => void verify()}>{keyOk ? <Check size={16} /> : null}{keyOk ? "Connected" : "Validate key"}</Button></Card><button className="text-link" onClick={() => { setSettings({ ...settings, provider: "local", model: "mistralai/Voxtral-Mini-3B-2507" }); next(); }}>Use a local provider instead</button></div> : null}
      {step === 2 ? <div className="onboarding-copy"><div className="step-icon"><ShieldCheck /></div><p className="eyebrow">Step 2</p><h1>Two permissions,<br />one-time only.</h1><p>Dictum needs your microphone to hear you and system input access to paste at your cursor.</p><div className="permission-list"><Card><div className="permission-icon"><Mic /></div><div><h3>Microphone</h3><p>Capture only while your shortcut is active.</p></div><Button variant="secondary" busy={micPermission === "checking"} onClick={() => void requestMicrophone()}>{micPermission === "ready" ? <Check size={16} /> : null}{micPermission === "ready" ? "Allowed" : "Allow"}</Button></Card>{isMac ? <Card><div className="permission-icon"><Keyboard /></div><div><h3>Accessibility</h3><p>Paste text into the app under your cursor.</p></div><Button variant="secondary" onClick={() => void openPermissions("accessibility")}>Open settings</Button></Card> : <Card><div className="permission-icon"><Keyboard /></div><div><h3>System-wide typing</h3><p>No extra setup is normally required on Windows or X11.</p></div><span className="permission-ok"><Check />Ready</span></Card>}</div>{micPermission === "error" && setupError ? <p className="field-error">{setupError}</p> : null}</div> : null}
